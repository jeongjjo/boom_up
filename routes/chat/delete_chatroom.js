...
module.exports = [
    '/chatroom', // URI : /test2 (최종은 /<route module>/<page module> : /sameple/test2)
    ...
    async function (req, res, next) {
        ...
        var a = await db.update("chatRoom",
            { _id: ObjectId(req.body.roomid) },
            { $push: { deleteInfo: req.user._id.toHexString() } });

        if (a) {
            var b = await db.update("chatMsg",
