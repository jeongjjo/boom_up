...
module.exports = [
    "/votecount/:kind/:id", // URI : /test2 (최종은 /<route module>/<page module> : /sameple/test2)
    ...
    async function (req, res, next) {
        ...
        var list = await db.get(kind, {_id: ObjectId(id)});

        var countUp=list.votingUp;
        var counDown=list.votingDown;
        var total=countUp+counDown;
        var standardccount=kind=='board'?50:30;
        var check=(standardccount<=total)?true:false;


        if(list) {
            return res.json({
                result : "success",
                data : {"total":total,"check":check}
            })
        } else {
            return res.json({
                result : "error",
