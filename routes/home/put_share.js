...
module.exports = [
    '/share/:id?', // URI : /test2 (최종은 /<route module>/<page module> : /sameple/test2)
    ...
    async function (req, res, next) {
        ...
        var updateBoard = await db.update("board", {_id:ObjectId(id)}, { $inc: { shareCnt:1 } }, { upsert: true });
        if(req.user){
            var updateUser = await db.update("user", {_id:req.user._id}, { $inc: { shareCnt:1 } }, { upsert: true });
        }
