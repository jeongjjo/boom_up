...
module.exports = [
    ...
    '/faqlist/:id?', // URI : /test2 (최종은 /<route module>/<page module> : /sameple/test2)
    ...
    async function (req, res, next) {
        ...
        var list = await db.getList('faq', where,0,50,{_id:-1 }) //todo
        for(var x=0; x<list.length; x++){
            list[x].A = list[x].a.replace(/\n/g, "<br>")
            list[x].Q = list[x].q.replace(/\n/g, "<br>")
        }
        res.render('left/_faq', {
            list: list || null
        });
    }
];
