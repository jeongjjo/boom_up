...
async function signup(deviceId, userId, token, nickname, req) {
  console.log(`OAUTH/SIGNUP/FACEBOOK,${userId},${token}`);
  var $where = {};
  $where[`auth.facebook.key`] = userId + "";

  try {
    // 사용자 가입 시 페북에 사용자 정보를 요청
    var response = await axios.get('https://graph.facebook.com/' + userId, {
      params: {
    ...
    if (response && response.status === 200 && response.data && response.data.id && response.data.name) {
      // 사용자 정보를 필요에 따라서 저장함
      ...
      if (userInfo) {
        ...
        try {
          ...
          console.error(err);
          console.error(`OAUTH/SIGNUP/FACEBOOK/FBCREATE,${userId},${token}`, req.ip, req.headers['user-agent'], userInfo._id.toHexString());
          return null;
        }
        ...
        if (!fbToken) {
          console.error(`OAUTH/SIGNUP/FACEBOOK/FAILFBTK,${userId},${token}`, req.ip, req.headers['user-agent'], userInfo._id.toHexString());
          return null;
        }
        ...
        console.warn('NO FACEBOOK USER');
      }
    } else {
      console.warn('FACEBOOK API FAIL');
    } // if response
  } catch (e) {
    console.error(e);
  }
...
async function signin(deviceId, userId, token, req) {
  console.log(`OAUTH/SIGNIN/FACEBOOK,${userId},${token}`);
  var $where = {};
  $where[`auth.facebook.key`] = userId + "";
  ...
  try {
    ...
    if (response && response.status === 200 && response.data && response.data.id && response.data.name) {
      ...
      if (userInfo && userInfo.use) {
        ...
        // } catch (err) {
        //   console.error(err);
        //   console.error(`OAUTH/SIGNIN/FACEBOOK/FBUPDATE,${userId},${token}`, req.ip, req.headers['user-agent'], userInfo._id.toHexString());
        //   return null;
        // }
        ...
        if (!fbToken) {
          console.error(`OAUTH/SIGNIN/FACEBOOK/FAILFBTK,${userId},${token}`, req.ip, req.headers['user-agent'], userInfo._id.toHexString());
          return null;
        }
...
async function addsignin(deviceId, origToken, userId, token, req) {
  console.log(`OAUTH/ADDSIGNIN/FACEBOOK,${userId},${token}`);
  var $where = {};
  $where[`auth.facebook.key`] = userId + "";
  ...
  try {
    ...
    if (response && response.status === 200 && response.data && response.data.id && response.data.name) {
      ...
      if (existsUser) {
        console.error(`OAUTH/ADDSIGNIN/FACEBOOK/NOUSERINDB,${userId},${token}`, req.ip, req.headers['user-agent']);
        return null;
      }
...
async function webSignup(req, res, rStat) {
  if (!rStat.nn) {
    return res.redirect('/auth/join/facebook?err=EINVALIDJOINSNS&r=' + encodeURIComponent(rStat.r || '/'));
  }
  ...
  try {
    ...
    if (responseUT && responseUT.access_token) {
      ...
      try {
        ...
        var response = await axios.get('https://graph.facebook.com/me', {
          params: {
            fields: "id,name,email",
            access_token: responseUT.access_token
          }
        });
        ...
        if (response && response.status === 200 && response.data && response.data.id && response.data.name) {
          ...
          // 사용자 정보를 필요에 따라서 저장함
          var $where = {};
          $where[`auth.facebook.key`] = response.data.id + "";
          ...
          if (userInfo) {
            ...
            try {
              ...
              // Firebase에 사용자가 없으면 생성
              console.error(err);
              return res.redirect('/auth/join/facebook?err=EFBERROR&r=' + encodeURIComponent(rStat.r || '/'));
            }
            ...
            if (!fbToken) {
              console.error(`OAUTH/CALLBACK/FACEBOOK/FAILFBTK,${userId},${responseUT.access_token}`, req.ip, req.headers['user-agent'], userInfo._id.toHexString());
              return res.redirect('/auth/join/facebook?err=EUNKNOWN&r=' + encodeURIComponent(rStat.r || '/'));
            }
            ...
            req.logIn(userInfo, function (err) {
              console.log('SIGNUP/FACEBOOK/WEB', req.ip, req.headers['user-agent'], responseUT.access_token, userInfo._id.toHexString(), 'OK');
              return res.redirect(rStat.r || '/');
            });
            return;
          } else {
            console.warn('NO FACEBOOK USER');
            return res.redirect('/auth/join/facebook?err=EFBFAIL&r=' + encodeURIComponent(rStat.r || '/'));
          }
...
async function webSignin(req, res, rStat) {
      ...
      try {
        ...
        if (response && response.status === 200 && response.data && response.data.id && response.data.name) {
          ...
          if (userInfo && userInfo.use) {
            ...
            console.warn('NO FACEBOOK USER');
            return res.redirect('/auth/login?ret=facebook&r=' + encodeURIComponent(rStat.r || '/'));
          }

        } else {
          console.warn('FACEBOOK API FAIL');
          return res.redirect('/auth/login?err=EFBFAIL&r=' + encodeURIComponent(rStat.r || '/'));
        } // if response
      } catch (e) {
        console.error(e);
        return res.redirect('/auth/login?err=EFBFAIL&r=' + encodeURIComponent(rStat.r || '/'));
  ...
  return res.redirect('/auth/login?err=EFBERROR&r=' + encodeURIComponent(rStat.r || '/'));
}
